...
window.openScheduleTodayModal = async function () {
    ...
    try {
        const medreps = await API.fetchMedreps();
        const activeReps = Array.isArray(medreps) ? medreps : [];

        if (!activeReps.length) {
            list.innerHTML = '<div class="st-empty">No med reps found.</div>';
            return;
        }
        ...
        const results = await Promise.all(
            activeReps.map(async rep => {
                try {
                    const data = await API.fetchScheduleByRep(rep.id);
                    ...
                    return { rep, allCalls, todayCalls };
                } catch {
                    return { rep, allCalls: [], todayCalls: [] };
                }
            })
...
// ── OPEN SCHEDULE LOGS ─────────────────────────────
window.stOpenLogs = function (idx) {
